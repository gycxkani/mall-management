...
<template>
  <div>
    <el-card>
      <el-button type="primary" size="small" @click="oppenDialog"
        >新增</el-button
      >
      <el-button type="danger" size="small" @click="delSkuAll"
        >批量删除</el-button
      >
      <!-- 表格 -->
      <el-table
    ...
    >
      <el-form
        ref="ruleFormRefAddSku"
        :model="ruleFormAddSku"
        :rules="rulesAddSku"
        label-width="120px"
      >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="addSkuOk"> 确定 </el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
const rulesAddSku = reactive({
  ...
  default: [{ required: true, message: "请输入规格数据", trigger: "blur" }],
});
...
const delSkuAll = async () => {
  const isdel = await ElMessageBox.confirm("是否删除选中项?", "批量删除", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).catch((err) => err);
