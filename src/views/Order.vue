  ...
  <div>
    <el-card>
      ...
      <el-row style="margin-top: 15px">
        <el-col>
          <el-button type="danger" size="small" @click="delOrderHandle"
            >批量删除</el-button
          >
      ...
      >
        ...
        <el-table-column label="操作" align="center">
          <template #default="{ row }">
            ...
            >
            <el-tag
              v-if="queryData.tab == 'noship'"
              type="warning"
              style="cursor: pointer"
              @click="orderSend(row.id)"
              >订单发货</el-tag
            >
            <el-tag
              type="warning"
              v-if="queryData.tab == 'refunding'"
              style="cursor: pointer; margin-right: 15px; margin-top: 15px"
              @click="refundHandle(row.id, 1)"
              >同意退款</el-tag
            >
            <el-tag
              type="warning"
    ...
    <el-dialog v-model="dialogVisibleOrder" title="订单详情" width="40%">
      <div class="orderMian">
        ...
        <div>
          ...
          <div class="orderCon">
            <div
              v-for="(item, i) in orderInfo.data.order_items"
              :key="i"
              style="
              ...
              "
            >
              商品名称：{{ item.goods_item ? item.goods_item.title : "已删除"
              }}<br />
              商品价格：{{ item.price }}<br />
              商品数量：{{ item.num }}<br />
              <span v-if="item.skuValue">
                ...
                <!-- 商品规格：{{ item.skuValue }} -->
                <el-tag
                  v-for="(sub, index) in item.skuValue"
                  :key="index"
        ...
        <div v-if="orderInfo.data.ship_data">
          ...
          <div class="orderCon">
            ...
            <el-button
              type="primary"
              size="small"
              @click="openLgstc(orderInfo.data.id)"
              >查看物流</el-button
            >
          </div>
        </div>
        <div v-if="orderInfo.data.address">
          ...
          <div class="orderCon">
            ...
            联系地址：
            {{
              orderInfo.data.address.province +
              orderInfo.data.address.city +
              orderInfo.data.address.district +
              orderInfo.data.address.address
            }}
          </div>
        </div>
      </div>
    ...
    <el-dialog v-model="dialogVisibleLgstc" title="物流信息" width="40%">
      ...
      <div style="margin-top: 30px">
        <el-timeline>
          <el-timeline-item
            v-for="(activity, index) in lgstc.list"
            :key="index"
...
const refundHandle = async (id, agree) => {
  console.log(id);
  if (agree == 1) {
    const isdel = await ElMessageBox.confirm("是否同意?", "退款", {
