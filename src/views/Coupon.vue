      ...
      <el-table :data="tableData" style="width: 100%" stripe>
        <el-table-column label="优惠券名称" width="330">
          <template #default="scope">
            <div
              :class="scope.row.statusVal == '领取中' ? 'coupon' : 'coupon1'"
            >
          ...
          <template #default="scope">
            <div>
              <span v-if="scope.row.type == 0"
                >满减 ￥{{ scope.row.value }}</span
              >
              <span v-if="scope.row.type == 1"
                >折扣 {{ scope.row.value }}折</span
              >
            </div>
          </template>
        </el-table-column>
            ...
            <div>
              <!--     删除按钮：未开始、已失效、已结束
                       ...
                       失效按钮：领取中
                                     -->
              <el-button
                v-if="scope.row.statusVal == '未开始'"
                type="primary"
                :icon="Edit"
                @click="oppenEdit(scope.row)"
                size="small"
              />
              <el-button
                v-if="scope.row.statusVal !== '领取中'"
                type="danger"
                :icon="Delete"
    ...
    <el-dialog v-model="dialogVisible" :title="TipsTitle" width="40%">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="addCouponOk"> 确定 </el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
const couponEnd = async (id) => {
  const isdel = await ElMessageBox.confirm("是否停止该优惠券？", "失效", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).catch((err) => err);
